...
class Undo {
    ...
    undo(reviver) {
        if (this.#past.length > 0) {
            const e = _a.applyEdit(this.#past.pop(), this.#present);
            this.#future.unshift(_a.diffScript(e, this.#present));
            this.#present = e;
        }
    ...
    redo(reviver) {
        if (this.#future.length > 0) {
            const e = _a.applyEdit(this.#future.shift(), this.#present);
            this.#past.push(_a.diffScript(e, this.#present));
            this.#present = e;
        }
        return this.#recover(this.#present, reviver);
    }
    static diffScript(older, newer) {
        ...
        function worker(e, f, i = 0, j = 0) {
            ...
            if (N > 0 && M > 0) {
                ...
                for (const h of _a.range(Math.floor(L / 2) + (L % 2 ^ 1) + 1)) {
                    for (const r of [0, 1]) {
                        let [c, d, o, m] = r === 0 ? [g, p, 1, 1] : [p, g, 0, -1];
                        for (const k of _a.range(-(h - 2 * Math.max(0, h - M)), h - 2 * Math.max(0, h - N) + 1, 2)) {
                            let a = k === -h ||
                                (k !== h &&
                                    c[_a.modulo(k - 1, Z)] < c[_a.modulo(k + 1, Z)])
                                ? c[_a.modulo(k + 1, Z)]
                                : c[_a.modulo(k - 1, Z)] + 1, b = a - k;
                            const [s, t] = [a, b];
                            while (a < N &&
                                b < M &&
                                e[(1 - o) * N + m * a + (o - 1)] ===
                            ...
                            if (L % 2 === o &&
                                z >= -(h - o) &&
                                z <= h - o &&
                                c[_a.modulo(k, Z)] + d[_a.modulo(z, Z)] >= N) {
                                const [D, x, y, u, v] = o === 1
                                    ? [2 * h - 1, s, t, a, b]
                                    : [2 * h, N - a, M - b, N - s, M - t];
                                if (D > 1 || (x !== u && y !== v)) {
                ...
                for (const n of _a.range(0, N)) {
                    result.push({ pos: i + n });
                }
            }
            else {
                for (const n of _a.range(0, M)) {
